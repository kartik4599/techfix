  ...
  return (
    <section className="px-4 sm:px-6 lg:px-8 py-8 sm:py-10">
      ...
      <Accordion type="single" collapsible className="w-full max-w-4xl mx-auto">
        {faqs.map((f, idx) => (
          <AccordionItem key={idx} value={`item-${idx}`} className="border-b border-[#d6e6d1]">
            <AccordionTrigger className="text-left text-sm sm:text-base font-medium py-4 sm:py-6 hover:no-underline">
              {f.question}
            </AccordionTrigger>
            <AccordionContent className="text-sm sm:text-base leading-relaxed pb-4 sm:pb-6 text-[#629550]">
              {f.answer}
            </AccordionContent>
          </AccordionItem>
        ))}
